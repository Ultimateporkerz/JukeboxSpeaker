...
public class JukeblockScreen extends AbstractContainerScreen<JukeblockMenu> {
    ...
    public boolean mouseDragged(double pMouseX, double pMouseY, int pButton, double pDragX, double pDragY) {
        if (dragging) {
            ...
            PacketDistributor.sendToServer(new C2SVolumeMessage(menu.getJukeblockPos(), newVolume));
            return true;
        }
    ...
    private boolean handleButtonClick(int mouseX, int mouseY) {
        ...
        if (isMouseOver(mouseX, mouseY, particlesLeftPos, particlesTopPos, particlesButtonWidth, particlesButtonHeight)) {
            if (areParticlesEnabled) {
                // Disable
                PacketDistributor.sendToServer(new C2SParticleMessage(menu.getJukeblockPos(), false));
            } else {
                // Enable
                PacketDistributor.sendToServer(new C2SParticleMessage(menu.getJukeblockPos(), true));
            }
            return true;
        }
        ...
        if (isMouseOver(mouseX, mouseY, playLeftPos, playTopPos, otherButtonWidth, otherButtonHeight)) {
            if (!isPlaying) {
                ...
                PacketDistributor.sendToServer(new C2SButtonMessage(menu.getJukeblockPos(), "PlayButton"));
                return true;
            }
        }
        ...
        if (isMouseOver(mouseX, mouseY, stopLeftPos, stopTopPos, otherButtonWidth, otherButtonHeight)) {
            if (!isStopped) {
                ...
                PacketDistributor.sendToServer(new C2SButtonMessage(menu.getJukeblockPos(), "StopButton"));
                return true;
            }
        }
        ...
        // Next
        if (isMouseOver(mouseX, mouseY, nextLeftPos, nextTopPos, otherButtonWidth, otherButtonHeight)) {
            PacketDistributor.sendToServer(new C2SButtonMessage(menu.getJukeblockPos(), "NextButton"));
            BetterMusicDiscs.jukeblockLOGGING("Next Track button clicked!");
            return true;
        }
        // Rewind
        if (isMouseOver(mouseX, mouseY, rewindLeftPos, rewindTopPos, otherButtonWidth, otherButtonHeight)) {
            PacketDistributor.sendToServer(new C2SButtonMessage(menu.getJukeblockPos(), "RewindButton"));
